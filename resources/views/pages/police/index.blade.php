...
@section('page-title', 'East Timor Police')
...
async function fetchPoliceData(filters = {}) {
    ...
    try {
        ...
        if (!res.ok) throw new Error(`Police API returned ${res.status}`);
        return await res.json();
    } catch (e) {
        console.error('Error fetching police:', e);
        return { polices: [], categoryCounts: {} };
    }
}
    ...
    data.forEach(police => {
        ...
        const popupContent = `
            ...
            <strong>Address:</strong>
                ...
                ${police.provinces_region ? ', ' + police.provinces_region : ''}, East Timor <br>
...
async function applyPoliceFilters() {
    ...
    const polices = Array.isArray(result.polices) ? result.polices : [];
